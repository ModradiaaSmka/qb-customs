...
--Blips
 ...
 Citizen.CreateThread(function()
     for k, v in pairs(bennyGarages1) do
...
function PreviewColour(paintType, paintCategory, paintID, r, g, b)
    local plyPed = PlayerPedId()
    local plyVeh = GetVehiclePedIsIn(plyPed, false)
    ...
    if paintType == 0 then --Primary Colour
        if paintCategory == 1 then --Metallic Paint
            ClearVehicleCustomPrimaryColour(plyVeh)
            SetVehicleColours(plyVeh, paintID, originalSecondaryColour)
            --SetVehicleCustomPrimaryColour(plyVeh, r, g, b) -- RGB
            SetVehicleExtraColours(plyVeh, originalPearlescentColour, originalWheelColour)
        else
            ClearVehicleCustomPrimaryColour(plyVeh)
            SetVehicleColours(plyVeh, paintID, originalSecondaryColour)
    ...
    elseif paintType == 1 then --Secondary Colour
        ClearVehicleCustomSecondaryColour(plyVeh)
        SetVehicleColours(plyVeh, originalPrimaryColour, paintID)
        --SetVehicleCustomSecondaryColour(plyVeh, r, g, b) -- RGB
...
function ApplyColour(paintType, paintCategory, paintID, r, g, b)
    local plyPed = PlayerPedId()
    local plyVeh = GetVehiclePedIsIn(plyPed, false)
...
RegisterNetEvent('bb_mechanic:admin', function()
    local ped = PlayerPedId()
    local pedCoord = GetEntityCoords(ped)
    local pedHeading = GetEntityHeading(ped)
    if IsPedInAnyVehicle(ped, false) then
        adminOnly = true
        bennyHeading = pedHeading
        if not isPlyInBennys then
            enterLocation(pedCoord)
        end
    end
end)
        ...
        if IsPedInAnyVehicle(plyPed, false) then
            ...
            for k, v in pairs(bennyGarages) do
                ...
                if nearDefault then
                    ...
                    if nearDefault then
                        if not isPlyInBennys then
                            Draw3DText(v.coords.x, v.coords.y, v.coords.z + 0.5, "[~p~E~w~ -  Mekanik Menu]", 255, 255, 255, 255, 4, 0.45, true, true, true, true, 0, 0, 0, 0, 55)
                            if IsControlJustReleased(1, 38) then
                                if (v.useJob and isAuthorized((QBCore.Functions.GetPlayerData().job.name), k)) or not v.useJob then
                                    ...
                                    QBCore.Functions.Notify("Bu mekaniği kullanma yetkin yok!", "error")
                                end
                            end
                        else
                            disableControls()
...
AddEventHandler("qb-customs:purchaseSuccessful", function()
    ...
    QBCore.Functions.Notify("Yükseltme satın alındı")
end)
...
AddEventHandler("qb-customs:purchaseFailed", function()
    ...
    attemptingPurchase = false
    QBCore.Functions.Notify("Yeterli paran yok!", "error")
end)

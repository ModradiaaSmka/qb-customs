window.onload = (e) => {
  ...
  function toggleMenuContainer(state) {
    if (state) {
      $("#menu_container").fadeIn(300, function () {
  ...
  function createMenu(menu, heading, subheading) {
    menuStructure[menu] = {
      ...
      container: "<ul id='menu_items' class='" + menu + "' style='display: none;'></ul>",
      items: {},
      itemsArray: {},
    }

    $("#menu_container .menu_divider").first().after(menuStructure[menu].container)
  }
  ...
  function populateMenu(menu, id, item, item2, r, g, b) {
    ...
    if (item2 == "none") {
      $("." + menu).append(
        "<li class='" + id + "' r='" + r + "' g='" + g + "' b='" + b + "'><span class='item1'>" + item + "</span></li>",
      )
    } else {
      $("." + menu).append(
        "<li class='" +
  ...
  function toggleMenu(state, menu) {
    if (state) {
      if (menuStructure[menu].selectedItem < maxMenuItems) {
        ...
        var val1 = $("." + menu + " .item_selected")
          .attr("class")
          .split(" ")[0]
        var val2 = $("." + menu + " .item_selected .item1").text()
        var val3 = $("." + menu + " .item_selected .item2").text()
        var r = $("." + menu + " .item_selected")
          .attr("r")
          .split(" ")[0]
        var g = $("." + menu + " .item_selected")
          .attr("g")
          .split(" ")[0]
        var b = $("." + menu + " .item_selected")
          .attr("b")
          .split(" ")[0]
        $.post(
          "https://qb-customs/selectedItem",
          JSON.stringify({
            id: val1,
            item: val2,
            item2: val3,
        ...
        for (var i = 0; i < Object.keys(menuStructure[menu].itemsArray).length; i++) {
          if (
            i > menuStructure[menu].selectedItem - maxMenuItems &&
            i <= maxMenuItems + (menuStructure[menu].selectedItem - maxMenuItems)
          ) {
            $("." + menu).append(menuStructure[menu].itemsArray[i])
          }
        }
      ...
      // Seçilen öğenin görünür olmasını sağla
      setTimeout(() => {
        var menuElement = document.querySelector("." + menu)
        if (menuElement) {
          var selectedElement = menuElement.querySelector(".item_selected")
          if (selectedElement) {
  ...
  function updateItem2Text(menu, id, text) {
    if (menuStructure[menu].previousSelectedItemID == null) {
      ...
      if (
        menuStructure[menu].itemsArray[prevID + 1] &&
        menuStructure[menu].itemsArray[prevID + 1].getElementsByClassName("item2").length > 0
      ) {
        menuStructure[menu].itemsArray[prevID + 1].getElementsByClassName("item2")[0].textContent =
          menuStructure[menu].items[prevID].item2
      }
  ...
  function updateItem2ID(menu, id) {
    menuStructure[menu].previousSelectedItemID = id
  }
  ...
  function scrollMenuFunctionality(direction, menu) {
    switch (direction) {
      case "down":
        if (
          menuStructure[menu].selectedItem < maxMenuItems - 1 &&
          ...
          menuStructure[menu].itemsArray[menuStructure[menu].selectedItem].classList.add("item_selected")
          menuStructure[menu].itemsArray[Object.keys(menuStructure[menu].itemsArray).length - 1].classList.remove(
            "item_selected",
          )
          var currentHTML = $("." + menu + " .item_selected").html()
          $("." + menu + " .item_selected").html("<i class='fas fa-angle-double-right'></i> " + currentHTML)
  ...
  function playSoundEffect(soundEffect, volume) {
    ...
    var Howl = window.Howl // Declare Howl variable
    ...
    if (typeof Howl !== "undefined") {
      ...
      console.error("Howler.js is not loaded. Please ensure Howler.js is included in your HTML.")
    }
  }
  ...
  window.addEventListener("message", (event) => {
    ...
    if (eventData.populateMenu) {
      populateMenu(eventData.menu, eventData.id, eventData.item, eventData.item2, eventData.r, eventData.g, eventData.b)
    }
    ...
    if (eventData.updateItem2ID) {
      updateItem2ID(eventData.menu, eventData.id)
    }
